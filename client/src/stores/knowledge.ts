...
import { getNodes, createNode, deleteNode as apiDeleteNode, streamChat } from '../api/knowledge'
...
export const useKnowledgeStore = defineStore('knowledge', () => {
  ...
  async function askQuestion(question: string, parentId?: string, context?: string) {
    ...
    try {
      ...
      rebuildChatHistoryFor(newNode._id)
  ...
  async function askQuestionStreaming(
    ...
    loading.value = true
    try {
      const newNode = await streamChat(
        { question, parentId, sessionId: sessionId.value, context },
        onEvent,
      )
      usingTestTree.value = false
      nodes.value.push(newNode)
      ...
      console.error('Failed to ask question (stream):', error)
      throw error
    } finally {
      loading.value = false
    }
  ...
  async function restoreSession(nodeId: string) {
    ...
    rebuildChatHistoryFor(nodeId)
  }

  function rebuildChatHistoryFor(nodeId: string) {
